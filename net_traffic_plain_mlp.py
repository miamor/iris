...
# Load dataset
with open('../Datasets/network_traffic/small_dataset.csv') as csvfile:
...
for row in dataset:
    row[8] = ["download", "multimedia"].index(row[8])
    row[:8] = [float(row[j]) for j in xrange(len(row))]

# Split x and y (feature and target)
random.shuffle(dataset)
datatrain = dataset[:int(len(dataset) * 0.8)]
datatest = dataset[int(len(dataset) * 0.8):]
train_X = [data[:8] for data in datatrain]
train_y = [data[8] for data in datatrain]
test_X = [data[:8] for data in datatest]
test_y = [data[8] for data in datatest]
...
input layer : 8 neuron, represents the feature of Traffic Flows
hidden layer : 8 neuron, activation using sigmoid
output layer : 2 neuron, represents the class of packet flow
...
for c in range(6, 12):
    neuron = [features_num, c, class_num] # number of neuron each layer
    print(neuron)
